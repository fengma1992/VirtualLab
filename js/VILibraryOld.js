...
VILibrary.VI = {
    ...
    AudioVI: {
        ...
        createVI: function (VICanvas) {
            ...
            //与其他VI的连接信息
            this.targetInfoArray = [];//[targetVIId, thisOutputType,targetInputType]二维数组，第二维分别存储输出VI的ID、自己的输出类型、targetVI的输入类型
            this.dataLine = 0;

            VILibrary.InnerObjects.existingVIArray.push(this);
            VILibrary.VI[this.name].logCount++;
            ...
            this.destroy = function () {
                ...
                let index = VILibrary.InnerObjects.existingVIArray.indexOf(_this);
                if (index !== -1) {
            ...
            let audioCtx = new (window.AudioContext || webkitAudioContext)(),
                source, analyser = audioCtx.createAnalyser(), animationNumber = 0;
    ...
    DCOutputVI: {
        ...
        createVI: function (VICanvas) {
            ...
            this.boxContent = '<div class="input-div"><span class="normal-span">初值:</span>' +
                '<input type="number" id="DCOutputVI-input" value="' + this.singleOutput + '" class="normal-input"></div>';
    ...
    AddVI: {
        ...
        createVI: function (VICanvas) {
            ...
            //与其他VI的连接信息
            this.sourceInfoArray = [];//[sourceVIId, sourceOutputType,thisInputType]二维数组，第二维分别存储输入VI的ID、输出类型、自己的输入类型
            this.targetInfoArray = [];//[targetVIId, thisOutputType,targetInputType]二维数组，第二维分别存储输出VI的ID、自己的输出类型、targetVI的输入类型
            this.dataLine = 0;
            ...
            this.updater = function () {
                ...
                if (_this.sourceInfoArray.length > 0) {

                    for (let sourceInfo of _this.sourceInfoArray) {
            ...
            this.setInitialData = function () {
                ...
                _this.setData(Number($('#AddVI-input').val()), 1);
            };
    ...
    FFTVI: {
        ...
        createVI: function (VICanvas) {
            ...
            this.setData = function (input) {
                ...
                _this.output = VILibrary.InnerObjects.fft(1, 11, input);
                return _this.output;
    ...
    SignalGeneratorVI: {
        ...
        createVI: function (VICanvas) {
            ...
            this.setInitialData = function () {
                ...
                _this.setSignalType(Number($('input[name=SignalGeneratorVI-type]:checked').val()));
            };
    ...
    BallBeamVI: {
        ...
        createVI: function (VICanvas, draw3DFlag) {
            ...
            let camera, scene, renderer, controls, markControl, base, beam, ball, mark, position = 0;
    ...
    RotorExperimentalRigVI: {
        ...
        createVI: function (VICanvas, draw3DFlag) {
            ...
            let camera, scene, renderer, controls, base, rotor, offSwitch, onSwitch, switchControl, loadedFlag = false,
                timer = 0, phase = 0, sampleFrequency = 8192, dt = 1 / sampleFrequency, disturbError = Math.random();
                ...
                switchControl.attachEvent('onclick', function () {
                    ...
                    if (!timer) {
                        ...
                        timer = window.setInterval(function () {
                            ...
                            rotor.rotation.x += 2 * Math.PI * _this.rotateFrequency * (60 / 10 + disturbError);
                            //定时更新相同数据线VI的数据
                            if (_this.dataLine) {
    ...
    BarVI: {
        ...
        createVI: function (VICanvas) {
            ...
            this.drawBackground = function () {
                ...
                if ((_this.container.height >= 200) && (_this.container.width >= 200)) {
                    ...
                    for (i = 0; i < scaleXNum; i += 2) {
                        ...
                        ctx.fillText(VILibrary.InnerObjects.fixNumber(temp), _this.offsetL + scaleXStep * i - 9 + _this.ratioX / 2, _this.container.height - 10);
                    }
                    //纵坐标刻度//
                    for (i = 2; i <= scaleYNum; i += 2) {
                        ...
                        ctx.fillText(VILibrary.InnerObjects.fixNumber(temp), _this.offsetL - 35, _this.offsetT + scaleYStep * i + 5);
                    }
                    ctx.closePath();
                    ctx.save();
                }
            ...
            this.drawRuler = function () {
                ...
                let curPointY = VILibrary.InnerObjects.fixNumber(_this.bufferVal[curPointX]);
            ...
            this.drawBackground = function () {
                ...
                if ((_this.container.height >= 200) && (_this.container.width >= 200)) {
                    ...
                    for (i = 2; i < scaleXNum; i += 2) {
                        ...
                        ctx.fillText(VILibrary.InnerObjects.fixNumber(temp), _this.offsetL + scaleXStep * i - 9, _this.container.height - 12);
                    }
                    //纵坐标刻度//
                    for (i = 2; i < scaleYNum; i += 2) {

                        temp = _this.maxValY - valStepY * i;
                        ctx.fillText(VILibrary.InnerObjects.fixNumber(temp), _this.offsetL - 28, _this.offsetT + scaleYStep * i + 5);
                    }
                    ctx.closePath();
                    ctx.save();
                }
    ...
    ButtonVI: {
        ...
        createVI: function (VICanvas) {
            ...
            this.draw = function () {
                ...
                _this.ctx.fillRect(0, 0, _this.container.width, _this.container.height);
                _this.ctx.fillStyle = 'black';
                _this.ctx.fillText(VILibrary.VI.ButtonVI.cnName, _this.container.width / 2 - 11, _this.container.height / 2 + 6);
            };
